...
GLint openGlGetUniformLocation(GLuint program, const GLchar *uniformName)
{
    GLint loc;
                        ...
                        GLuint *program)
{
	...
	status *= openGlCompileShader(GL_VERTEX_SHADER, 1, &vertSource, &vertShader);
    
    // Create and compile fragment shader
	status *= openGlCompileShader(GL_FRAGMENT_SHADER, 1, &fragSource, &fragShader);
    ...
    // Link program
	status *= openGlLinkProgram(prog);
    
#if defined(DEBUG)    
    // Validate program
	status *= openGlValidateProgram(prog);
#endif
	...
	{	
        for(i = 0; i < uniformNameCt; i++)
		{
            if(strlen(uniformNames[i]))
			    uniformLocations[i] = openGlGetUniformLocation(prog, uniformNames[i]);
		}
		*program = prog;
	}
